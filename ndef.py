...
class Ndef():
    ...
    def __init__(self):
        ...
        self.header = {
            'MB' : 1, 'ME' : 1, 'CR' : 0, 'SR' : 1, 'IL' : 0, 'TNF' : '{0:03b}'.format(self.tnf)
        }


    def getTNF(self):
        return '{0:03b}'.format(self.tnf)
    ...
    def Well_Known(self, payload):
        ...
        text = self.Well_Known_Text(payload, "en")
        sp = self.Well_known_URI(payload)

        if uri :
            self.tnf = bytes([85])
            self.header['TNF'] = '{0:03b}'.format(1)
            return bytes([1,len(payload)-4]) + self.tnf + uri
        # elif sp :
            # self.tnf = bytes([83,112])
            #     self.len_tnf = len(self.tnf)
            #     return bytes([1,len(payload), self.len_tnf]) + smartp

        elif text:
            self.tnf = text
            self.header['TNF'] = '{0:03b}'.format(1)
            return bytes([1,len(payload)+len("en")+1]) + text
        return None
    ...
    def getNdef_payload(self, payload):
        ...
        if len(payload) < 1024 :
            ...
            if empty:
                self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + empty + bytearray(payload.encode("UTF-8"))

            # elif well_known and not uri:
            #     self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + well_known + bytearray(payload.encode("UTF-8"))

            elif mime :
                self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + mime + bytearray(payload.encode("UTF-8"))

            elif uri :
                self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + uri + bytearray(payload.encode("UTF-8"))

            elif external:
                self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + external + bytearray(payload.encode("UTF-8"))

            elif unchanged :
                self.ndef_payload = bytearray(self.bitstring_to_bytes(header)) + unchanged + bytearray(payload.encode("UTF-8"))
            print(self.ndef_payload)
            return self.ndef_payload
        return None

Ndef().getNdef_payload("http://www.google.com")
