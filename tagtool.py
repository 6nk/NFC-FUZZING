...
class TagTool(cli.CommandLineInterface):
    ...
    def prepare_tt3_tag(self, target):
        print("options.size", self.options.size)
        ...
        try:
            ...
            if self.options.input:
                print("options.input",self.options.input)
                try:
                    self.options.data = self.options.input.buffer.read()
                except AttributeError:
                    self.options.data = self.options.input.read()
        ...
        if not (hasattr(self.options, "tt3_data") and self.options.keep):
            if self.options.input:
                ...
                print("OPTION.DATA prepared ", self.options.data)
                ndef_area_size = ((ndef_data_size + 15) // 16) * 16
                print("options.size 2222 ", self.options.size)
            ...
            print("self.options.ver", self.options.ver)
            attribute_data[0] = self.options.ver
            attribute_data[1] = self.options.nbr
            attribute_data[2] = self.options.nbw
            print("self.options.max", self.options.max)
            ...
            print("attribute_data[3:5] ", attribute_data[3:5] )
            print("[self.options.rfu]", [self.options.rfu])
            attribute_data[5:9] = 4 * [self.options.rfu]
            print("attribute_data[5:9]", attribute_data[5:9])
            print("self.options.wf", self.options.wf)
            attribute_data[9] = self.options.wf
            print("self.options.data", self.options.data)
            print("attribute_data[9]",attribute_data[9])
            attribute_data[10:14] = struct.pack(">I", len(self.options.data))
            print("len(self.options.data)", len(self.options.data))
            print("attribute_data[10:14]", attribute_data[10:14])
            attribute_data[10] = self.options.rw
            print("attribute_data[10]", attribute_data[10])
            attribute_data[14:16] = struct.pack(">H", sum(attribute_data[:14]))
            print("attribute_data[14:16]", attribute_data[14:16])
            print("len", len(attribute_data))
            print("attribute_data", attribute_data)
            self.options.tt3_data = attribute_data + ndef_data_area
            print("tt3_data prepare_tag = ",self.options.tt3_data)
            print("tt3_data type", type(self.options.tt3_data))
        idm = bytes.fromhex(self.options.idm)
        print("IDM = ",self.options.idm)
        pmm = bytes.fromhex(self.options.pmm)
        print("PMM = ",self.options.pmm)
        _sys = bytes.fromhex(self.options.sys)
        print("SYS = ",self.options.sys)
